...
class OrderLogServiceTest {
        ...
        public void dailyActions() {
                ...
                ordersRepository.save(
                                Orders.builder().orderId(1L).orderNumber(1L).orderDatetime(yesterday.toString())
                                                .build());
                List<OrderProduct> orderProducts = new ArrayList<>();
                orderProducts.add(
                                OrderProduct.builder().orderId(1L).productId(1L).name("아메리카노").amount(1).size("small")
                                                .temperature("ice")
                                                .build());
                orderProducts.add(
                                OrderProduct.builder().orderId(1L).productId(2L).name("카라멜 마키아또").amount(2)
                                                .size("small")
                                                .temperature("hot")
                                                .build());
                orderProducts.add(
                                OrderProduct.builder().orderId(1L).productId(3L).name("롱블랙").amount(3).size("small")
                                                .temperature("ice")
                                                .build());
                orderProducts.add(OrderProduct.builder().orderId(1L).productId(4L).name("프렌치 아메리카노").amount(4)
                                .size("small")
                                .temperature("ice").build());
                orderProducts.add(
                                OrderProduct.builder().orderId(1L).productId(6L).name("카페 라떼").amount(1).size("small")
                                                .temperature("ice")
                                                .build());
                orderProducts.add(
                                OrderProduct.builder().orderId(1L).productId(7L).name("초콜릿 라떼").amount(1).size("large")
                                                .temperature("ice")
                                                .build());
                orderProducts.add(OrderProduct.builder().orderId(1L).productId(11L).name("민트 블렌드 티").amount(1)
                                .size("large")
                                .temperature("hot").build());
                orderProducts.add(
                                OrderProduct.builder().orderId(1L).productId(12L).name("블루밍 프루트 유스베리 티").amount(2)
                                                .size("small")
                                                .temperature("hot").build());
                orderProducts.add(
                                OrderProduct.builder().orderId(1L).productId(16L).name("딸기 주스").amount(1).size("small")
                                                .temperature("ice")
                                                .build());
                // 주문상품 저장
                ...
                paymentRepository.save(
                                Payment.builder().paymentId(1L).orderId(1L).method(CARD).totalPrice(10000)
                                                .build());
